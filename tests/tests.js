...
assert(isBufferEqual(block.data[0], blocka_0));

let blocka_0_hash = new Uint8Array(fromHex(sha256(blocka_0)));
let raw_principal_hash = new Uint8Array(fromHex(sha256(raw_principal)));
console.log('raw principal hash', toHex(raw_principal_hash));
let blocka_0_combined_hash = new Uint8Array(fromHex(sha256(mergeUInt8Arrays(raw_principal_hash, blocka_0_hash))));
